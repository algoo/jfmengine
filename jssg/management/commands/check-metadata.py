...
class MetadataStatus:
    ...
    def get_metadata_status_for(cls, page):
        ...
        metadata_status.complete = (metadata_status.missing == []) and (
            metadata_status.empty == []
        )
        if len(settings.JFME_CONTENT_REQUIRED_METADATA) > 0:
            metadata_status.progression = (
                (
                    len(settings.JFME_CONTENT_REQUIRED_METADATA)
                    - len(metadata_status.missing)
                    - len(metadata_status.empty)
                )
                * 100
                / len(settings.JFME_CONTENT_REQUIRED_METADATA)
            )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for page in Page.load_glob(
            path=list(map(lambda p: Path(p).absolute(), options["content path"])),
            all=True,
        ):
